...
describe('ConversationAreaInviteList', () => {
  ...
  const expectProperlyRenderedAreaInvites = async (
    ...
    if (noActiveInvitesExist) {
      ...
      const tableRows = renderData.getAllByLabelText('convAreaRequestsTableRow');
      // the number of table rows should match the length of the conversation area invites
  ...
  describe('When there are no active conversation areas', () => {
    ...
    it('displays a No Current Requests text when there are no active areas', async () => {
  ...
  describe('When there are active conversation areas but no active invitations', () => {
    ...
    it('Does not display a table when there are zero area invites', async () => {
      const renderData = await renderConversationAreaList();
      await expectProperlyRenderedAreaInvites(renderData, allActiveAreas, []);
    });
    it('displays a No Current Requests text when there are no invites', async () => {
      const renderData = await renderConversationAreaList();
      await expectProperlyRenderedAreaInvites(renderData, allActiveAreas, []);
    });
  });
  describe('When there are active areas and active invitations', () => {
    it('displays 2 table rows when there are two area invites', async () => {
      const renderData = await renderConversationAreaList();
      await expectProperlyRenderedAreaInvites(renderData, allActiveAreas, allInvitations);
    });
    it('updates the number of table rows when the area invites change', async () => {
      const renderData = await renderConversationAreaList();
      await expectProperlyRenderedAreaInvites(renderData, allActiveAreas, allInvitations);
